...
process.on('uncaughtException', (err) => {
    if (err && (err.name === 'CustomError' || String(err.message).includes('missing required'))) {
        ...
        // writeServerLog('warn', 'ProtoDecode', `Decode error: ${err.message}, instance: ${JSON.stringify(err.instance)}`);
        return;
    }
...
async function main() {
    ...
    const disableServerLogging = process.env.DISABLE_SERVER_LOGGING === '1' || process.env.DISABLE_SERVER_LOGGING === 'true';
    if (enableAppLogging) {
        console.log('Application logging is enabled and will log to application.log');
    }
    if (disableServerLogging) {
        console.log('Server logging is disabled and will not log to server.log');
    }
    
    // if (skipRustConnect) {
    //     console.log('Skipping Rust connection on startup due to SKIP_RUST_CONNECT environment variable');
    // }
